          ...
          <section className="home-view screen-enter" aria-labelledby="home-heading">
            <div className="hero-copy">
              ...
              <div className="hero-actions">
                <button className="button primary hero-primary" type="button" onClick={() => go('work')}>SEE THE WORK <span aria-hidden="true">→</span></button>
                <button className="button text-button" type="button" onClick={() => go('challenge')}>TRY THE METHOD</button>
              </div>
              <div className="proof-rail" aria-label="Selected proof">
                <div><strong>29 courses / month</strong><span>AI-ENABLED PRODUCTION WORKFLOW</span></div>
                <div><strong>10% lower turnover</strong><span>LEARNING TIED TO BUSINESS OUTCOME</span></div>
                <div><strong>Live adoption tools</strong><span>BUILT AROUND REAL MOMENTS OF WORK</span></div>
              </div>
            </div>
            <div className="hero-visual" aria-label="A mission path representing Cesar's AI enablement method">
              <div className="mission-map" aria-hidden="true"><span className="map-pin">●</span><span className="map-line line-one"/><span className="map-line line-two"/><span className="map-line line-three"/><span className="map-flag">⚑</span></div>
              <div className="portrait-frame"><img src={headshot} alt="Professional headshot of Cesar Ramos" /></div>
        ...
        {view === 'work' && (
          <section className="work-view screen-enter" aria-labelledby="work-heading">
            ...
            <div className="work-hero">
              <div><h1 id="work-heading">Four problems. Four interventions. One adoption method.</h1><p>Start with the outcome. Open a case study when you want the evidence, workflow, and interactive example behind it.</p></div>
...
function CaseDetail({ study, onBack }: { study: (typeof caseStudies)[number]; onBack: () => void }) {
  ...
  return (
    <section className="case-view screen-enter" aria-labelledby="case-heading">
      ...
      <div className="case-demo-stage"><div className="case-demo-copy"><span>INTERACTIVE PROOF</span><h2>Try the intervention.</h2><p>This recreation demonstrates the workflow or learning experience without exposing employer branding, private data, or production systems.</p></div><RepresentativePreview kind={study.slug} /></div>
