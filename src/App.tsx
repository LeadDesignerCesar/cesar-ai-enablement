      ...
      <main id="main" className="view-stage">
        {view === 'home' && (
          <section className="home-view screen-enter" aria-labelledby="home-heading">
            ...
            <div className="method-ticker" aria-label="My enablement method">{method.slice(0, 4).map((item, index) => <span key={item}>{String(index + 1).padStart(2, '0')} &nbsp; {item}{index < 3 && <b>→</b>}</span>)}</div>
          </section>
        )}
...
function AboutLeaderboard({ entries, score }: { entries: LeaderboardEntry[]; score: number }) {
  ...
  return <aside className="about-leaderboard" aria-label="Getting to know Cesar leaderboard"><div className="leaderboard-head"><div><span>GETTING TO KNOW CESAR</span><strong>{youIndex === 0 ? 'You took the lead.' : `${clicksToLead} more ${clicksToLead === 1 ? 'click' : 'clicks'} to take the lead.`}</strong></div><b>YOUR SCORE {score.toLocaleString()}</b></div><div className="leaderboard-list">{entries.map((entry, index) => <div key={entry.label} className={entry.label === 'You' ? 'leaderboard-you' : ''}><span>{index + 1}</span><strong>{entry.label}</strong><b>{entry.score.toLocaleString()}</b></div>)}</div></aside>
}
