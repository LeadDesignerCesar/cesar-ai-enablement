      ...
      <div className={`challenge-layout ${step === 5 ? 'results-layout' : ''}`}>
        <div className="challenge-main">
          ...
          {step === 2 && <Stage number="02" eyebrow="BUILD THE FIX" title="What kind of support removes the friction?">
            ...
            <NavigationRow onBack={() => setStep(1)} nextLabel="CONTINUE" nextDisabled={!solution} onNext={() => setStep(3)} />
          </Stage>}
          ...
          {step === 3 && <Stage number="03" eyebrow="ENABLE THE PEOPLE" title="How will people get confident enough to use it?">
            ...
            <NavigationRow onBack={() => setStep(2)} nextLabel="CONTINUE" nextDisabled={!adoption} onNext={() => setStep(4)} />
          </Stage>}
          ...
          {step === 4 && <Stage number="04" eyebrow="PROVE THE IMPACT" title="What evidence would prove it worked?">
            ...
            <NavigationRow onBack={() => setStep(3)} nextLabel="BUILD MY BLUEPRINT" nextDisabled={!measure} onNext={() => setStep(5)} />
          </Stage>}
...
function NavigationRow({ onBack, backLabel = 'PREVIOUS', nextLabel, nextDisabled = false, onNext }: { onBack: () => void; backLabel?: string; nextLabel: string; nextDisabled?: boolean; onNext: () => void }) { return <div className="page-nav challenge-page-nav"><button className="page-back" type="button" onClick={onBack}>← {backLabel}</button><button className="page-next" type="button" disabled={nextDisabled} onClick={onNext}>{nextLabel} →</button></div> }
