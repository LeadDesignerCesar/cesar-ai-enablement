  ...
  return (
    <section className="production-infographic" aria-label="AI-enabled course production workflow">
      ...
      <div className="prod-flow">
        {steps.map((step, index) => <div className="prod-step-wrap" key={step.n}><article className={`prod-step step-${index + 1}`}><div className="prod-icon" aria-hidden="true">{step.icon}</div><span>{step.n}</span><strong>{step.title}</strong><small>{step.detail}</small></article>{index < steps.length - 1 && <i className="prod-arrow" aria-hidden="true">→</i>}</div>)}
      </div>
      <div className="prod-lower">
        <div className="prod-operating">
          <span>THE OPERATING MODEL</span>
          <h4>Build one course while reviewing another. Every day.</h4>
          <div className="prod-timeline">
            <article><b>DAY 01</b><strong>Build Course 01</strong><small>Review queue begins</small></article>
            <article><b>DAY 02</b><strong>Build Course 02</strong><small>Review Course 01</small></article>
            <article><b>DAY 03</b><strong>Build Course 03</strong><small>Review Course 02</small></article>
            <div className="timeline-gap" aria-hidden="true">•••</div>
            <article className="day-final"><b>DAY 29</b><strong>Build Course 29</strong><small>Core curriculum complete</small></article>
          </div>
          <div className="prod-continuous">✓ Continuous iteration with SME validation all month long.</div>
        </div>
        <aside className="prod-outcomes" aria-label="Production outcomes">
          <span>THE OUTCOME</span>
          <div className="outcome-grid">
            <div><strong>29</strong><small>courses completed</small></div>
            <div><strong>1</strong><small>month</small></div>
            <div><strong>SME + CESAR</strong><small>owned accuracy + final decisions</small></div>
            <div><strong>AI</strong><small>accelerated production</small></div>
          </div>
        </aside>
      </div>
...
function EventAppDemo({ compact }: { compact: boolean }) {
  ...
  return (
    <div className="rep-event-interactive">
      <div className="event-demo-intro"><span>TRAINING SUMMIT EXPERIENCE</span><strong>Click through the app.</strong><p>This recreation demonstrates the experience architecture: personalized logistics when timing matters, then participation mechanics that make the summit more social and useful.</p><div className="event-demo-points"><span>TIME-AWARE</span><span>PERSONALIZED</span><span>GAMIFIED</span><span>SOCIAL</span></div></div>
      <div className="interactive-phone" aria-label="Interactive representative Event App">
        <div className="phone-status"><span>{now.time}</span><b>● ● ●</b></div><div className="app-top"><button type="button" onClick={() => setScreen('home')} aria-label="Go to Event App home">SUMMIT</button><span>REPRESENTATIVE DEMO</span></div>
        {screen === 'home' && <div className="app-screen home-screen"><div className="moment-switcher" aria-label="Preview how the landing page changes during the day"><button type="button" className={moment === 'morning' ? 'active' : ''} onClick={() => setMoment('morning')}>8:35</button><button type="button" className={moment === 'breakout' ? 'active' : ''} onClick={() => setMoment('breakout')}>10:42</button><button type="button" className={moment === 'afternoon' ? 'active' : ''} onClick={() => setMoment('afternoon')}>2:18</button></div><div className="live-now"><span>RIGHT NOW</span><strong>{now.label}</strong><p>Go to <b>{now.room}</b></p><small>{now.detail}</small></div><div className="app-action-grid"><button type="button" onClick={() => setScreen('agenda')}><b>AGENDA</b><span>See the day</span></button><button type="button" onClick={() => setScreen('questions')}><b>ASK</b><span>Send a question</span></button><button type="button" onClick={() => setScreen('qr')}><b>SCAN</b><span>Find QR clues</span></button><button type="button" onClick={() => setScreen('leaderboard')}><b>LIVE</b><span>Leaderboard</span></button><button type="button" onClick={() => setScreen('photos')}><b>PHOTO</b><span>Share a moment</span></button></div></div>}
